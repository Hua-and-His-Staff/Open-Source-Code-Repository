    ...
    try:
        while total_steps < args.total_steps:
            ...
            for _ in range(args.rollout_steps):
                ...
                steps.append(( rewards, EV_actions, EV_probs, AC_actions, AC_probs, values))
                if dones.sum()>0:
                    break
...
def process_rollout(args, steps, device, dones):
    ...
    for t in reversed(range(len(steps) - 1)):
        rewards,EV_actions, EV_probs, AC_actions, AC_probs,values = steps[t]
        ...
        log_action_probs = (EV_probs.log_prob(EV_actions).sum(1, True) + AC_probs.log_prob(AC_actions).sum(1, True))/5
