...
class MADDPG(object):
    ...
    def update(self, sample, agent_i, parallel=False, logger=None):
        ...
        if self.alg_types[agent_i] == 'MADDPG':
            ...
            for a in range(self.nagents):
                t0_next_obs[a] = torch.tensor(np.zeros((next_obs[0].shape[0], 3)), dtype=torch.float)
            ...
            for n in range(self.nagents):               # for each agents
                for b in range(next_obs[0].shape[0]):   # for the number of batches
                    t0_next_obs[n][b][:] = next_obs[n][b][0:3]
       ...
       #     else:
        ...
        t0_obs = [[], [], []]
        for a in range(self.nagents):
            t0_obs[a] = torch.tensor(np.zeros((obs[0].shape[0], 3)), dtype=torch.float)
        for n in range(self.nagents):  # for each agents
            for b in range(obs[0].shape[0]):  # for the number of batches
                t0_obs[n][b][:] = obs[n][b][0:3]
        ###################################################
    ...
    def init_from_env(cls, env, agent_alg="MADDPG", adversary_alg="MADDPG",
        ...
        alg_types = [adversary_alg if atype == 'adversary' else agent_alg for
                     atype in ['agent', 'agent', 'agent']]#env.agent_types]
        b = [Box(low=-np.inf, high=+np.inf, shape=(18,), dtype=np.float32),Box(low=-np.inf, high=+np.inf, shape=(18,), dtype=np.float32),Box(low=-np.inf, high=+np.inf, shape=(18,), dtype=np.float32)]
