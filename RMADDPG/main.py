...
def run(config):
    ...
    #    obs = np.array([[obs[0],obs[1],obs[2]]])
        obs = np.array([[obs[0][0:3],obs[1][0:3],obs[2][0:3]]])
        ...
        obs_history = np.empty([1,3,18])
        next_obs_history = np.empty([1,3,18])
    #    obs_history = np.empty([1,3,12])
    #    next_obs_history = np.empty([1,3,12])
        ...
        for et_i in range(config.episode_length):
            ...
            rewards = np.array([[rewards[0],rewards[1],rewards[2]]])
            action_list_0.append(actions[0][0][0])
            action_list_1.append(actions[0][1][0])
            action_list_2.append(actions[0][2][0])
            SOC_list_0.append(next_obs[0][2])
            SOC_list_1.append(next_obs[1][2])
            SOC_list_2.append(next_obs[2][2])
            ############### WHICH REWARD TO USE ##############
            ...
            dones = np.array([[[dones[0]],[dones[1]],[dones[2]]]])
            reward_0+=g_rewards[0][0]
            reward_1+=g_rewards[0][1]
            total_reward+=sum(g_rewards[0])
            ...
            '''
        #    next_obs = np.array([[next_obs[0],next_obs[1],next_obs[2]]])
            next_obs = np.array([[next_obs[0][0:3],next_obs[1][0:3],next_obs[2][0:3]]])
            ...
            if not rnn:
                ...
                g_storage_buffer.push(obs_history, agent_actions, rewards, next_obs_history, dones)
    ...
    #        config.episode_length * config.n_rollout_threads)
     ...
     #       logger.add_scalar('agent%i/mean_episode_rewards' % a_i, a_ep_rew, ep_i)
        ...
        print(total_reward)
        total_reward_list.append(total_reward[0])
    t = {"mean": total_reward_list}
    with open(os.path.join("total_reward_list_pomdp"+".pkl"), "wb") as f: 
        pickle.dump(t, f, pickle.HIGHEST_PROTOCOL)   
    a = {"mean": action_list_0}
    with open(os.path.join("action_list_0"+".pkl"), "wb") as f: 
        pickle.dump(a, f, pickle.HIGHEST_PROTOCOL) 
    s = {"mean": SOC_list_0}
    with open(os.path.join("SOC_list_0"+".pkl"), "wb") as f: 
...
def parse_arguments():
    ...
    parser.add_argument("--init_noise_scale", default=0.1, type=float)
...
if __name__=="__main__":
    ...
    rnn = False    # also need to set this inside MADDPG class. Make args later.
    if rnn:
        history_steps = 6
