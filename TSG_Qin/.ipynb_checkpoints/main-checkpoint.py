...
def main():
    ...
    log_dir = os.path.join(args.log_dir, 'one_value')
    save_dir = os.path.join(args.save_dir, 'one_value')        
    try:
        os.makedirs(log_dir)
    except OSError:
        pass    
    ...
    envs = make_vec_envs(args.train_seed, args.num_processes, args.env_settings)
    actor_critic = Policy(
    ...
    try:
        while j < num_updates:
            ...
            if (j % args.save_interval == 0 or j == num_updates - 1):
                total_num_steps = (j + 1) * args.num_processes * args.num_steps
                mean_eval_reward = evaluate(actor_critic, args.evaluate_seed, args.num_processes, device, args.env_settings)
                writer.add_scalar('Evaluate/reward', mean_eval_reward, total_num_steps)
                #writer.add_scalar('Evaluate/user', mean_user_reward, total_num_steps)
