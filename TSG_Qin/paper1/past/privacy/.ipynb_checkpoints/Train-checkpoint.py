    ...
    for i in range(test_runs):
        with torch.no_grad():
            ...
            for step in range(int(1438)):
                ...
                mu, sigma, AC_prob = actor_net(observations)
                values = critic_net(observations)
                normal_probs = dist.normal.Normal(mu,sigma+1e-4)
                continuous_actions = normal_probs.sample()
                #压缩到0.1-0.9之间
                AC_prob = 0.1 + 0.8*AC_prob
                Bernoulli_probs = dist.bernoulli.Bernoulli(AC_prob)
                discrete_actions = Bernoulli_probs.sample()
    ...
    try:
        while total_steps < args.total_steps:
            ...
            for _ in range(args.rollout_steps):
                ...
                steps.append(( costs, continuous_actions, normal_probs, discrete_actions, Bernoulli_probs, values))
                if dones.sum()>0:
                    break
            ...
            log_normal_probs, log_Bernoulli_probs, normal_entropys, Bernoulli_entropys, values, returns, advantages = process_rollout(args, steps, device, Dones)
            policy_loss = args.AC_number*(log_Bernoulli_probs * advantages[:,-10:]).mean() + args.EV_number*(log_normal_probs[:,-5:]*advantages[:,2].unsqueeze(1)).mean() + (log_normal_probs[:,0] * advantages[:,0]).mean() + (log_Bernoulli_probs*advantages[:,1].unsqueeze(1)).mean() + (log_normal_probs*advantages[:,1].unsqueeze(1)).mean()
            value_loss = (.5 * (values - returns) ** 2.).mean() 
            entropy_loss = args.EV_number*normal_entropys.mean() + args.AC_number*Bernoulli_entropys.mean()
            ...
            writer.add_scalar('Policy/EV_advantages', advantages[:,2].mean(), total_steps)
            writer.add_scalar('Policy/EV_values', values[:,2].mean(), total_steps)
            writer.add_scalar('Policy/EV_returns', returns[:,2].mean(), total_steps)
...
def process_rollout(args, steps, device, dones):
    ...
    advantages = torch.zeros(args.num_workers, args.AC_number+3)
    advantages = advantages.to(device)
    out = [None] * (len(steps) - 1)
    # run Advantage Estimation, calculate returns, advantages
    for t in reversed(range(len(steps) - 1)):
        costs, continuous_actions, normal_probs, discrete_actions, Bernoulli_probs, values = steps[t]
        ...
        log_normal_probs = normal_probs.log_prob(continuous_actions)
        log_Bernoulli_probs = Bernoulli_probs.log_prob(discrete_actions)

        out[t] = log_normal_probs, log_Bernoulli_probs, normal_probs.entropy(), Bernoulli_probs.entropy(), values, returns, advantages
