    ...
    for i in range(test_runs):
        with torch.no_grad():
            ...
            for step in range(int(1438)):
                ...
                reward_sum += np.mean(gen_cost + BES_cost[:,0] + np.sum(AC_costs, axis=1))
...
def train(args, actor_net, critic_net, actor_optimizer, critic_optimizer, env, device):
    ...
    try:
        while total_steps < args.total_steps:
            ...
            policy_loss = (30*(log_AC_probs * AC_advantages).mean() + (log_genEV_probs[:,0].unsqueeze(1) * gen_advantages).mean() + 30*(log_AC_probs * BES_advantages).mean() + 21*(log_genEV_probs * BES_advantages).mean())/51
            value_loss = ((.5 * (gen_value - gen_returns) ** 2.).mean() + (.5 * (BES_value - BES_returns) ** 2.).mean() +30*(.5 * (AC_values - AC_returns) ** 2.).mean())/32
