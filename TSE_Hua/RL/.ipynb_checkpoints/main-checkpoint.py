...
parser.add_argument('--rollout-steps', type=int, default=10, help='steps per rollout')
parser.add_argument('--training-start-steps', type=int, default=1e5, help='start step')
parser.add_argument('--training-day', type=int, default=1, help='select which day to train')
parser.add_argument('--total-steps', type=int, default=int(4e8), help='total number of steps to train for')
parser.add_argument('--save-interval', type=int, default=int(1e3), help='interval for model saving')
...
parser.add_argument('--actor-lr', type=float, default=1e-4, help='learning rate')
parser.add_argument('--critic-lr', type=float, default=1e-3, help='learning rate')
...
for i in range(args.num_workers):
    env_fns.append(lambda: EIEnv(env_id=args.training_day,gamma=args.gamma))
venv=SubprocVecEnv(env_fns)

actor_net = Actor(venv.observation_space.shape[0]+1,venv.action_space.shape[0])
critic_net = Critic(venv.observation_space.shape[0]+1,venv.action_space.shape[0])
