...
class EIEnv(gym.Env):
    def __init__(self,gamma=0.9):
        ...
        self.mu = 3*np.load('./Data/mu.npy')[:N]
#obs: t[0], P_PV-PLsum[1], P_G[2], SOC[3], T_out[4] E_EV[5:5+M], AC_open_time[-N:]
#actions: u_G, (P_EV)*M, (u_{AC})*N 
        ...
        self.observation_scalar=lambda x:x/np.concatenate((np.array([24,10,self.max_g,1,10]), self.PEVmax, 10*np.ones(N)),axis=0)
...
#action: u_G, (P_EV)*M, (u_{AC})*N 
        ...
        #calculate EV costs
        EV_costs = 0
        P_EV = self.EV_sign*action[1:1+M]*self.PEVmax
        for i in range(M):
            if self.EV_sign[i]==True:
                if self.E_EV[i]<=0:
                    P_EV[i]=0
                if self.end_charge_time[i]<=t:
                    EV_costs += ReLU(self.E_EV[i])**2
        ...
        for i in range(N):
            ...
            #若超过3度，接管控制，并惩罚与控制相反的控制信号
                ...
                AC_costs[i] += 2*(1-action[1+M+i]) + 0.2*(np.exp(3*self.mu[i]))
                AC_actions[i] = 1
            elif self.T_in[i]<=self.Tlower[i]-3:
                AC_costs[i] += 2*(action[1+M+i]) + 0.2*(np.exp(3*self.mu[i]))
                AC_actions[i] = 0
            else:
                #温度在Tlower~Tupper之间没有损失
                Tdiff = np.abs(self.T_in[i] - np.clip(self.T_in[i], self.Tlower[i], self.Tupper[i]))
                AC_costs[i] = 0.1*(np.exp( self.mu[i]*Tdiff))
                AC_actions[i] = action[1+M+i]
        gen_cost = 0.5*self.PG + 1e-2*self.PG**2 
        P_AC = AC_actions*self.PACmax
        P_BES = self.PV[self.index]- self.load[self.index] + self.PG - np.sum(P_EV) - np.sum(P_AC) 
        bes_cost = 0
        ...
        self.soc = new_soc
        #E_EV
        self.E_EV -= self.EV_sign*P_EV*self.dt
        self.T_in -= (self.AC_a*(self.T_in-Tout) + self.AC_b*P_AC)*self.dt
