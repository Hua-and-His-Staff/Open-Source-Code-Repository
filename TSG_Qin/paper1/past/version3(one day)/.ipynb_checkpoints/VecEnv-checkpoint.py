...
class EIEnv(gym.Env):
    ...
    def step(self,action):
        ...
        if self.index==self.total_step-1:
            self.done=True        
        #print(self.state)
        return self._get_obs(), 0.2*gen_cost+bes_cost+np.sum(comfort_cost), self.done, {'fail':self.fail}
...
def worker(remote_addr, env_wrapper):
    ...
    try:
        while True:
            ...
            if cmd == 'step':
                observation, cost, done, info = env.step(data)
                observation=env.observation_scalar(observation)
                local_conn.send((observation, cost,  done, info))
            elif cmd == 'multistep':
                observation, cost, done, info = env.multiple_step(*data)
                observation=env.observation_scalar(observation)
                local_conn.send((observation, cost, done, info))             
            elif cmd == 'reset':
...
class SubprocVecEnv(object):
    ...
    def step(self, actions):
        ...
        obs, cost, dones, infos = zip(*results)

        return np.stack(obs), np.stack(cost), np.stack(dones), infos
