...
class EIEnv(gym.Env):
    ...
    def step(self,action):
        ...
        #action: u_G, (P_EV)*n, (u_{AC})*m 
        action = np.clip(action,0,a_max=None)
        u_g = action[0]
        #AC
        ...
        for i in range(30):
            ...
            elif Tin[i]>=self.Tupper[i]+3:
                comfort_cost[i] += 2*(1-action[1+n+i]) + 0.2*(np.exp(5*self.mu[i]))
                AC_action[i] = 1
            elif Tin[i]<=self.Tlower[i]-3:
                comfort_cost[i] += 2*(action[1+n+i]) + 0.2*(np.exp(5*self.mu[i]))
                AC_action[i] = 0
            else:
                #温度在Tlower~Tupper之间没有损失
                Tdiff = np.abs(Tin[i] - np.clip(Tin[i], self.Tlower[i], self.Tupper[i]))
                comfort_cost[i] = 0.1*(np.exp( self.mu[i]*Tdiff))
                AC_action[i] = action[1+n+i]
                
        P_AC = AC_action*self.PACmax
        ...
        if self.index==self.total_step-1:
            self.done=True        
        #print(self.state)
        return self._get_obs(), 0.2*gen_cost, bes_cost, comfort_cost, self.done, {'fail':self.fail}
...
def worker(remote_addr, env_wrapper):
    ...
    try:
        while True:
            ...
            if cmd == 'step':
                observation, gen_cost, bes_cost, comfort_costs, done, info = env.step(data)
                observation=env.observation_scalar(observation)
                local_conn.send((observation, gen_cost, bes_cost, comfort_costs, done, info))
            elif cmd == 'multistep':
                observation, gen_cost, bes_cost, comfort_costs, done, info = env.multiple_step(*data)
                observation=env.observation_scalar(observation)
                local_conn.send((observation, gen_cost, bes_cost, comfort_costs, done, info))             
            elif cmd == 'reset':
...
class SubprocVecEnv(object):
    ...
    def step(self, actions):
        ...
        obs, gen_costs, bes_costs, comfort_costs, dones, infos = zip(*results)

        return np.stack(obs), np.stack(gen_costs),np.stack(bes_costs),np.stack(comfort_costs), np.stack(dones), infos
