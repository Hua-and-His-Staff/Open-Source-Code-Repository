...
class MG():
    def __init__(self,assign_credit,privacy_preserving):
        ...
        if self.privacy_preserving:
            ...
            #state: t, Ppure, PG, SOC, Tout, EVs_demand, ACs_T, ACs_sign
            #index:[0], [1], [2], [3], [4],   [5:5+M],  [-2*N:-N], [-N:]
            #actions: u_G, (P_EV)*M, (P_AC)*N 
            self.observation_space = Box(low=np.float32(np.zeros(5+M+2*N)),high=np.float32(np.ones(5+M+2*N)))
            self.action_space = Box(low=np.float32(np.zeros(1+M+N)), high=np.float32(np.ones(1+M+N)))
            self.obs_scalar=lambda x:(x-np.concatenate((np.array([0,0,0,0,20]), np.zeros(M),20*np.ones(N),np.zeros(N)),axis=0))/np.concatenate((np.array([24,20,self.max_g,1,10]),10*np.ones(M+N), np.ones(N)),axis=0)
    ...
    def move(self,actions):
        ...
        #AC
        ACs_action = actions[-N:]
        self.ACs_sign = (self.ACs_start<=step) & (self.ACs_end>=step)
        fines[-N:] += (self.ACs_sign*((self.ACs_T>=self.ACs_parameter[3]+5)*(1-ACs_action) + (self.ACs_T<=self.ACs_parameter[4]-5)*ACs_action)) + 0.2*(1-self.ACs_sign)*(ACs_action)
        ...
        self.episode_reward += -costs.sum() - BES_cost
        #self.users_reward += - costs[1:].sum()
        #credit assignment
        values = -costs-fines
        values[0] -= BES_cost
        ...
        return self._get_obs(), values if self.assign_credit==True else (-costs-fines).sum()-BES_cost, self.done, self.episode_reward if self.done==True else None
    ...
    def _get_obs(self):
        ...
        if self.privacy_preserving:                       
            ...
            return np.concatenate((obs, self.EVs_demand, self.ACs_T, self.ACs_sign), axis=0)
...
class MG_for_test():
    ...
    def move(self,actions):
        ...
        return self._get_obs(), costs, self.done, {'P_EV':EVs_power, 'P_AC':ACs_power}
    ...
    def reset(self):
        self.index = self.total_steps*(self.num_days-1)
        self.DG_power = 5
        self.soc = 0.1+0.8*np.random.rand()
        self.EVs_start = self.EVs_parameter[3] + np.random.randint(12,size=M)
        self.EVs_end = self.EVs_start+np.random.randint(low=3*12,high=4*12,size=M)
