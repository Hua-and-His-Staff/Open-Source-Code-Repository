...
def costU(u):
    ...
    # finfo函数 是根据括号中的类型来获得信息，获得符合这个类型的数型
...
class EIEnv(gym.Env):
    def __init__(self,env_id=0,gamma=0.99):
        ...
        # #定义 __init__方法，这里有三个参数，这个self指的是一会创建类的实例的时候这个被创建的实例本身（例中的self）
        # 你也可以写成其他的东西，比如写成me也是可以的，这样的话下面的self.Name就要写成me.Name
    ...
    try:
        while True:
            ...
            if cmd == 'step':
                ...
                print('multisitep', observation, reward, done, info)
                observation=env.observation_scaler(observation)
                local_conn.send((observation, reward, done, info))
            elif cmd == 'reset':
