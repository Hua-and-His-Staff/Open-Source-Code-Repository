    ...
    for i in range(test_runs):
        with torch.no_grad():
            ...
            for step in range(int(1440)):
                ...
                observation, reward, dones, infos = env.step(actions.detach().cpu().numpy())
                reward_sum += reward
    ...
    try:
        while total_steps < args.total_steps:
            ...
            for _ in range(args.rollout_steps):
                ...
                bes_cost = torch.from_numpy(bes_cost).float().unsqueeze(1)
                comfort_cost = torch.from_numpy(comfort_cost).float().unsqueeze(1)
                gen_cost = gen_cost.to(device)
                bes_cost = bes_cost.to(device)
                comfort_cost = comfort_cost.to(device)
                steps.append(( gen_cost, bes_cost, comfort_cost, EV_actions, EV_probs, AC_actions, AC_probs, values))
                if dones.sum()>0:
                    break
            ...
            log_action_probs, entropys, values, returns, advantages = process_rollout(args, steps, device, Dones)
...
def process_rollout(args, steps, device, dones):
    ...
    _, _, _, _, _, _, _, last_values = steps[-1]
    ...
    for t in reversed(range(len(steps) - 1)):
        gen_cost, bes_cost, comfort_cost, EV_actions, EV_probs, AC_actions, AC_probs,values = steps[t]
        _, _, _, _, _, _, _, next_values = steps[t + 1]
        #if done
        #returns = rewards
        returns = rewards + returns * args.gamma 
        #returns = returns * args.gamma 
        advantages = rewards + next_values.detach() * args.gamma - values.detach()
        log_gen_action_probs = (EV_probs.log_prob(EV_actions).sum(1, True) + AC_probs.log_prob(AC_actions).sum(1, True))/5

        entropys = EV_probs.entropy().sum(1,True) + AC_probs.entropy().sum(1,True)
        out[t] = log_action_probs, entropys, values, returns, advantages
